...
class DataSet(object):
    ...
    def __init__(self, measurement):
        self.filename = measurement.filename
        self.setup = measurement.setup
    ...
    @classmethod
    def gen(cls, measurement, getter):
        """Create a dataset designed to store parameters in getter."""
        data_set = cls(measurement)
        for attr in getter.attrs:
            data_set.add(DataArray(np.full(measurement.shape, np.nan),
                                   attr.name, attr.units))
